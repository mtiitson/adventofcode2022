const input = `[[6,[[0],[6,7],[4,10,8,4,1]],1,[[2,6],0,[4,6,2,3],9]]]
[[10,[7],6]]
...
[[[10,[10,7,9],[],[8,9]],7],[2,10,[0,9,[4,2,10,7]],6,9],[[1,2,3],0],[5,3,[2],2]]`.split('\n\n').map(pair => pair.split('\n').map(eval));
...
const compare = (l, r) => {
    ...
    while (l.length && r.length) {
        const res = compare(l.shift(), r.shift());
...
const part1 = input.reduce((acc, [l, r], i) => compare(l, r) ? acc + i + 1 : acc, 0);
console.log(part1);

const ordered = [...input.flat(), [[2]], [[6]]]
    .sort((a, b) => compare(a, b) ? -1 : +1)
    .map(JSON.stringify);
const part2 = (ordered.indexOf("[[2]]") + 1) * (ordered.indexOf("[[6]]") + 1);
console.log(part2);
